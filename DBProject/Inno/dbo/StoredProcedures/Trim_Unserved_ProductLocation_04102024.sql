...
CREATE PROCEDURE [dbo].[Trim_Unserved_ProductLocation_04102024] 
AS
BEGIN
    ...
    BEGIN
        ...
        set c = 0
        where 
        -- not exists (select * from adx_bom where adx_bom.CHILD_ID = adx_productlocation.PRODUCT_ID and adx_bom.ALT_GROUP <> '-') and
...
----------------------------------------------- AND
        ...
        where  
            exists (select * from adx_bom where adx_bom.PARENT_ID = adx_productlocation.PRODUCT_ID and adx_bom.ALT_GROUP = '-')
