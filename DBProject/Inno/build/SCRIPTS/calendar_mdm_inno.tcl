...
proc Build_Period_Managers {} {
	...
	set fp [open "$path(data)/adx_period_manager_inno.csv" r];
	gets $fp;
...
proc Build_Capacity_Patterns {} {
	...
	set fp [open "$path(data)/adx_cappattern_inno.csv" r];
	gets $fp;
...
proc Build_CFI {} {
	...
	set fp [open "$path(data)/adx_cfi_family_inno.csv" r]
	gets $fp
   	while {[gets $fp linestring] > 0} {
		if {$linestring eq {}} {
			continue; #blank line
...
proc Build_Locations {} {
	...
	#
	set fp [open "$path(data)/adx_location_inno.csv" r]
   	gets $fp
   	while {[gets $fp linestring] > 0} {
		if {$linestring eq {}} {
			continue; #blank line
