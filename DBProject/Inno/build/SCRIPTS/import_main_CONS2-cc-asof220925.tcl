...
#source c:/Adexa/demos/fbdemo/SCRIPTS/import_mdm.tcl
#source c:/Adexa/adm/semicon/SCP/SCRIPTS/import_main_CONS2-cc.tcl
##############################################################################
##
...
proc run {} {
	...
	set_modes;
	#
	filepm save ./data/FABLESS_CONS2_UNSOLVED_MODEL_V70618\_$fdate\_$ftime.data
	BalanceAndSched
	#
	postSolve;
	#Rebuild Cube Reports
	tk_cube_model_init
	filepm save ./data/FABLESS_CONS2_SOLVED_MODEL_V70618\_$fdate\_$ftime.data
...
proc add_UDAs {} {
    ...
    demand attribute add_string ALLOCATION_S
    lot attribute add_string ALLOCATION_S
    lot attribute add_string ALLOCATION
    method attribute add_string ALLOCATION
    supply attribute add_string ALLOCATION
...
proc Initialize {} {
	...
	set path(main) "c:/Adexa/adm/semicon/SCP";
	#set path(data) "$path(main)/DATA-demo";
	...
	source $path(script)/calendar_mdm.tcl
	#source $path(script)/holidays_mdm.tcl
	source $path(script)/create_per_mgr.tcl
	source $path(script)/resource_mdm.tcl
	source $path(script)/process_mdm.tcl
	source $path(script)/product_mdm.tcl
	source $path(script)/bom_mdm.tcl
	#source $path(script)/customers_mdm.tcl
	#Supply for AMD demo
	source $path(script)/supply_mdm_AMD.tcl
	#Demand for AMD demo
	#source $path(script)/demand_mdm_AMD.tcl
	source $path(script)/demand_mdm_demo.tcl
    ...
    source $path(script)/wip_mdm.tcl
    source $path(script)/safety_stock_mdm.tcl
    #Solve Script for AMD model
    source $path(script)/solve_model_20201209.tcl;
...
proc Import_MDM {} {
	...
	tool_generate_starts_constraints "$path(data)/adx_graybox.csv"; #when using tool_constraints.tcl
	echo "After Greybox";
...
proc adhocMethods {} {
 ...
 #       demand attribute add_string WIRE_S
        ...
        method_alt elem delete [method_alt elem get id]
       #############################################################
       method_alt set id FG3@FG-DC1
       method_alt elem set ix 1
       method_alt elem set pref 0
       method_alt elem set ix 2
       method_alt elem set pref 0
...
proc adhocData {} {
	...
	lot grade_decomposition set
        ...
        #############################################################
        #############################################################
        #Modify probe lot sizes for one part - Renasis Demo
        inv set id PROBE3@ASSY-M2
        inv set incr_lot 1
        inv set min_lot 1
        inv set id PROBE3@ASSY-M3
        inv set incr_lot 1
        inv set min_lot 1
        inv set id PROBE4@ASSY-M2
        inv set incr_lot 1
        inv set min_lot 1
        inv set id PROBE4@ASSY-M3
        inv set incr_lot 1
        inv set min_lot 1
        inv set id PROBE3@WPROB-M1
        inv set incr_lot 1
        inv set min_lot 1
        inv set id PROBE3@WPROB-M2
        inv set incr_lot 1
        inv set min_lot 1
        inv set id PROBE4@WPROB-M1
        inv set incr_lot 1
        inv set min_lot 1
        inv set id PROBE4@WPROB-M2
        inv set incr_lot 1
        inv set min_lot 1
	...
	supply set id Inv-1
	supply attribute_value set DEMAND_ID "FREE"
	supply set id Inv-2
	supply attribute_value set DEMAND_ID "FREE"
	supply set id Inv-7
	supply attribute_value set DEMAND_ID "FREE"
	supply set id Inv-8
	supply attribute_value set DEMAND_ID "FREE"
	#supply set id Inv-13
	#supply attribute_value set DEMAND_ID "FREE"
	#supply set id Inv-14
	#supply attribute_value set DEMAND_ID "FREE"

	lot set id WIP-20
	lot attribute_value set DEMAND_ID "FREE"
	lot set id WIP-21
	lot attribute_value set DEMAND_ID "FREE"
