...
proc Connect_MethAlt_to_Product {} {
   	...
   	set fp [open "$path(data)/adx_productlocation.csv" r];
   	gets $fp
   	while {[gets $fp linestring] > 0} {
		...
		com@location set method_alt "$ProdId@$Loc";
   	}
   	close $fp
}
#
proc Build_Vendor {} {
	...
	global errfp path;
	#
	set fp [open "$path(data)/adx_vendor.csv" r];
	gets $fp;
	#
	while {[gets $fp linestring] > 0} {
		#echo $linestring
...
proc Build_Purchase_Method {} {
   	...
   	set fp [open "$path(data)/adx_method_buy.csv" r]
   	gets $fp
...
proc Build_Method_Make {} {
	...
	#
   	set fp [open "$path(data)/adx_method_make.csv" r]
   	#gets $fp;
	gets $fp headstring;
	set obj method;
	...
	for {set l_ix 0} {$l_ix < [llength $lProdId]} {incr l_ix} {
		...
		#
		set lAltBom [lsearch -all $altBomParent $ProdId];
		if {[llength $lAltBom] > 0} {
			for {set b_ix 0} {$b_ix < [llength $lAltBom]} {incr b_ix} {
				set altBomId [lindex $altBom [lindex $lAltBom $b_ix]];
				set mthdId [format "M_%s_%s_%s" $ProdId $Route $altBomId];
				...
				com set id $ProdId;
				set pArea [com attribute_value get PROD_AREA];
				if {$pArea != "-"} {
					method attribute_value set PROD_AREA $pArea;
				}
				#if {$assyloc != "-"} {
			...
			set mthdId "M_$ProdId\_$Route\_$BoMId";
			method add_manufacture $mthdId $Loc;
			method set id $mthdId;
			#echo "...ProdId=$ProdId,BoMId=$BoMId,Route=$Route,dmSpec=$dmSpec";
			method set bom $BoMId;
			method set route $Route;
			if {$Cost != "-"} {
				method attribute_value set COST_DIRECT $Cost;
				if {$Cost > 0} {
...
proc Build_Method_Move {} {
   	...
   	set fp [open "$path(data)/adx_transportation.csv" r]
...
proc Build_Route {} {
   	...
   	set fp [open "$path(data)/adx_route.csv" r]
   	gets $fp
...
proc Build_RouteSeg {} {
   	...
   	set fp [open "$path(data)/adx_operation.csv" r]
   	gets $fp
...
proc Build_Op_Overrides {} {
   	...
   	set fp [open "$path(data)/adx_op_override.csv" r]
   	gets $fp
   	while {[gets $fp linestring] > 0} {
		if {$linestring eq {}} {
			continue; #blank line
