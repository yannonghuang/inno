...
proc Build_Resources {} {
	...
	set fp [open "$path(data)/adx_resource.csv" r]
	gets $fp
...
proc Build_ResCapInter {} {
	...
	set fp [open "$path(data)/adx_rescapinter.csv" r];
	gets $fp;
	while {[gets $fp linestring] > 0} {
		if {$linestring eq {}} {
			continue; #blank line
...
proc Build_ResCapExc {} {
	...
	set fp [open "$path(data)/adx_rescapexc.csv" r];
	gets $fp;
	while {[gets $fp linestring] > 0} {
		if {$linestring eq {}} {
			continue; #blank line
...
proc Build_Resource_Alt_Groups {} {
   	...
   	set fp [open "$path(data)/adx_resalt.csv" r]
   	gets $fp
...
proc Build_Bill_of_Resource {} {
   	...
   	set fp [open "$path(data)/adx_bor.csv" r]
   	gets $fp
...
proc Build_BoR_Alternatives {} {
   	...
   	set fp [open "$path(data)/adx_bora.csv" r]
   	gets $fp;
